...
import numpy as np
import os
import time  # NUEVO: para medir tiempos y FPS
...
# ==============================
# Si está en True, el programa se comporta "como si" fuese una Pi:
#   - solo hace inferencia en 1 de cada N frames
#   - muestra un FPS de inferencia más realista
SIMULATE_RASPBERRY = True

# Procesar solo 1 de cada N frames (por ejemplo, 1 de cada 3)
...
# Contadores para simulación y FPS
...
while cap.isOpened():
    ...
    if SIMULATE_RASPBERRY and (frame_count % RASPI_PROCESS_EVERY_N_FRAMES != 0):
        # En modo simulación, mostramos el frame sin hacer inferencia
        ...
        cv2.imshow("YOLO TFLite (Sim RPi)", frame_display)

        if cv2.waitKey(1) & 0xFF == ord("q"):
            break

        # Saltamos el resto del loop (no se llama al modelo)
        continue
    ...
    # Mostrar FPS de inferencia (equivalente Pi)
    cv2.putText(
        frame_display,
